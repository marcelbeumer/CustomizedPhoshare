...
class ExportFile(object):
    ...
    def check_iptc_data(self, export_file, options, is_original=False, file_updated=False):
        ...
        if not options.dryrun: # marcelb: removed all those "cache/changed" if-statements
            title=self.photo.caption # marcelb: added title support
            exiftool.update_iptcdata(export_file, new_caption, new_keywords,
                                        ...
                                        iptc_data.image_height, hierarchical_subject=[], title=title)
            return True
        return False
